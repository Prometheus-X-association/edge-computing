...
Options:
    ...
    -f  Force and rebuild PDC configuration.
    -h  Display help.
EOF
}
...
if [ "${FORCE_RECREATE}" = "true" ] || [ ! -d "${DS_PDC_DIR}" ] || [ -z "$( ls -A "${DS_PDC_DIR}" )" ] ; then
    log "Cleanup PDC config..."
...
log "Start PDC..."
pushd "${DS_PDC_DIR}"
    docker compose down -v && docker compose up -d --no-build --force-recreate
popd
...
if [ "$(jq '.code' <<<"${RESP}")" -ne 200 ]; then
    ...
    echo "Received bearer token: ${TOKEN}"
fi
