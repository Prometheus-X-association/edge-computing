...
The application uses the [Keras](https://keras.io/) python framework to train a _deep convolutional neural network_ (
CNN)
model based on `Keras`' [MNIST digits classification dataset](https://keras.io/api/datasets/mnist/).

#### [GBC](../src/samples/gbc)

The application uses the [scikit-learn](https://scikit-learn.org/stable/) python package to train a _Gradient Boosting
Classifier_ (GBC) machine learning
model based on `sklearn`'
s [Olivetti faces dataset](https://scikit-learn.org/stable/datasets/real_world.html#the-olivetti-faces-dataset).
