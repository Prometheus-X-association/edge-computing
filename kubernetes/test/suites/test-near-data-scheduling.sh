...
testNearDataSchedulingWithNodeAffinity() {
    LOG "Create near-data restricted pod..."
    kubectl -n "${PTX}" apply -f "${ROOT_DIR}"/test/manifests/ptx-edge-pdc_collocated_test_pod.yaml
    kubectl -n "${PTX}" wait --for="condition=Ready" --timeout=60s pod/"${POD}"
    assertTrue "Pod creation failed!" "$?"
    #
    kubectl get nodes -L "${PZ_LAB}/zone-A" -L "${PZ_LAB}/zone-B" -L "${PZ_LAB}/zone-C" -L "${PDC_LAB}/enabled"
