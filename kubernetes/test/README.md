...
Testing of the PTX-Edge extension on the Kubernetes ecosystem is
fundamentally performed using primarily
...
- **Level 4**: Create a DiD test Kubernetes setup, where the REST-API endpoints
...
and perform a simple test deployment on a temporary Kubernetes
...
Test cases are composed of different unit tests at different levels.
...
To run all test scripts in the `suite` folder, use the following script:
...
Each subproject defines its own test cases.
