...
# Dependencies
DEPS=(docker k3d kubectl kubecolor helm skopeo)
...
function setup_test_cluster() {
    ...
    k3d cluster create "$TEST_K8S" --wait --timeout="${TIMEOUT}s" --servers=1 --agents=1 \
    ...
    kubectl version
    echo
    kubecolor -n kube-system get all
    echo -e "\n>>> K3s nodes:\n"
    kubecolor get nodes -o wide -L ${PZ_LAB}/zone-A -L ${PZ_LAB}/zone-B
}
